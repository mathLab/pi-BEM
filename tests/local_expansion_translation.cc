...
int main ()
{
  ...
  // to test the local expansion translation, we create a second local expansion centered
  // in point Q''(-1,-1,-1)
  Point<3> new_new_center(-1.0,-1.0,-1.0);
  LocalExpansion new_local(truncation_order, new_new_center, alf_ptr);


  // and fill the newest empty expansion with the contribution of the previously created
  // local expansion, making use of the Add(const LocalExpansion &parent) member function
  new_local.Add(local);
  ...
  deallog<<"The relative multipole approximation error is: "<<fabs(exact_potential-approx_potential)/fabs(exact_potential)<<std::endl;
