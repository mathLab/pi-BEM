...
main()
{
  ...
  // we also import a CAD geometry of a portion of a cylinder to integrate on
  // curved cell
  std::string cad_filename =
    std::string(SOURCE_DIR) + "/testData/Revolution_1.iges";
  ...
  // and --- possibly --- with MappingManifold. But there is some work to do to
  // make it work
  //  std::vector<TopoDS_Face> occ_faces;
  //  std::vector<TopoDS_Edge> occ_edges;
  //  std::vector<TopoDS_Vertex> occ_vertices;
  //  OpenCASCADE::extract_geometrical_shapes(surface, occ_faces, occ_edges,
  //  occ_vertices); OpenCASCADE::NURBSPatchManifold<2,3> manifold(occ_faces[0],
  //  1e-7);
  ...
  // Case 4.2b
  Point<2> singularity_location_in_parametric_plane_2(0.66 * .5 + .5,
                                                      0.0 * .5 + .5);
  // Case 4.2c
  Point<2> singularity_location_in_parametric_plane_3(0.66 * .5 + .5,
                                                      0.66 * .5 + .5);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      deallog << "Result : " << I[0][2] * 4 * dealii::numbers::PI
              << "    Exact: " << -0.343807 * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Abs Err: "
              << -0.343807 * 4 * dealii::numbers::PI -
                   I[0][2] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(-0.343807 * 4 * dealii::numbers::PI -
                      I[0][2] * 4 * dealii::numbers::PI) /
                   fabs(-0.343807 * 4 * dealii::numbers::PI)
              << std::endl;
      ...
      deallog << std::setprecision(15);
      deallog << "Result : " << I[0][2] * 4 * dealii::numbers::PI
              << "    Exact: " << -0.497099 * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Abs Err: "
              << -0.497099 * 4 * dealii::numbers::PI -
                   I[0][2] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(-0.497099 * 4 * dealii::numbers::PI -
                      I[0][2] * 4 * dealii::numbers::PI) /
                   fabs(-0.497099 * 4 * dealii::numbers::PI)
              << std::endl;
      ...
      deallog << std::setprecision(15);
      deallog << "Result : " << I[0][2] * 4 * dealii::numbers::PI
              << "    Exact: " << -0.877214 * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Abs Err: "
              << -0.877214 * 4 * dealii::numbers::PI -
                   I[0][2] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(-0.877214 * 4 * dealii::numbers::PI -
                      I[0][2] * 4 * dealii::numbers::PI) /
                   fabs(-0.877214 * 4 * dealii::numbers::PI)
              << std::endl;
      ...
      //    deallog<<"Abs Err:
      //    "<<-2.114175-I[0][0]*4*dealii::numbers::PI<<std::endl; deallog<<"Rel
      //    Err:
      //    "<<fabs(-2.114175-I[0][0]*4*dealii::numbers::PI)/fabs(-2.114175)<<std::endl;


      //    strong integrals case b
      //    deallog<<"Abs Err:
      //    "<<-1.935711-I[0][0]*4*dealii::numbers::PI<<std::endl; deallog<<"Rel
      //    Err:
      //    "<<fabs(-1.935711-I[0][0]*4*dealii::numbers::PI)/fabs(-1.935711)<<std::endl;

      //    strong integrals case c
      //    deallog<<"Abs Err:
      //    "<<.8790179-I[0][0]*4*dealii::numbers::PI<<std::endl; deallog<<"Rel
      //    Err:
      //    "<<fabs(.8790179-I[0][0]*4*dealii::numbers::PI)/fabs(.8790179)<<std::endl;
    }
}
