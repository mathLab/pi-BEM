...
Element Method.
* It is derived from ParameterAcceptor to have a common interface with parameter
files. In
* particular this class performs the matrix vector products approximating long
range interactions
...
public:
  ...
  void
  generate_multipole_expansions(
    const TrilinosWrappers::MPI::Vector &phi_values,
    const TrilinosWrappers::MPI::Vector &dphi_dn_values) const;
  ...
  /// We need to call this function after having generating the multipole
  /// expansions. Since this function takes a considerable amount of time we
  /// have chosen to parallelise it using multithreaded TBB on a single node and
  /// MPI to allow even a greater level of parallelism. In this case we don't
  /// need any communication because we have made sure that the multipole
  /// expansions are replicated on each node. Thus we can safely split the
  /// descending phase.
  void
  ...
  /// In this function we actually build up the final preconditioner taking
  /// great care of all the constraints, through a constraint matrix, we need to
  /// impose on our system. We need the vector alpha to properly build up the
  /// band of the BEM system we are going to invert (with ILU in this case).
  /// This function takes a lot of time to properly compute the preconditioner
  /// sparsity pattern and fill it. For this very reason we have taken great
  /// care in providing a double parallelisation strategy. Firstly we split
  /// among different nodes the computations with MPI and then we use TBB to
  /// ensure a parallelisation between different threads on multicore
  /// architectures.
