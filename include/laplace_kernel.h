...
namespace LaplaceKernel
{
  ...
  Tensor<2,dim> hypersingular(const Point<dim> &R)
  {
    Tensor<2,dim> Hyper;
    switch (dim)
      {
      ...
      case 3:
	{
	  for(unsigned int i=0;i<dim;++i)
		...
		for(unsigned int k=0;k<dim && k!=i;++k)
	      ...
	      else Hyper[i][j]=3*R[i]*R[j];
	 return Hyper/( -4*numbers::PI * R.square() * R.square() * R.norm() );
	}
  ...
  template <int dim>//mio//
  void kernels(const Tensor<1, dim> &R,Tensor<2,dim> &H, Tensor<1,dim> &D, double &d)
  {
    double r = R.norm();
    double r2 = r*r;
    switch (dim)
