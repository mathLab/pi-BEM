...
main()
{
  ...
  // we then avaluate the exact potential phi = sum_i
  // (a_i*(P_i-Q)*n_i/|P_i-Q|^3) evaluated at point Q(0,0,0)
  Point<3> zero(0.0, 0.0, 0.0);
  double   exact_potential_normal_gradient = 0.0;
  for (unsigned int i = 0; i < charges_locations.size(); ++i)
    {
      exact_potential_normal_gradient +=
        -charges_intensities[i] *
        (charges_locations[i] * normals_at_charges_locations[i]) /
        pow(charges_locations[i].distance(zero), 3.0);
      deallog << "Charge " << i + 1 << " of intensity "
              << charges_intensities[i] << " is located at point ("
              << charges_locations[i] << ")  and has normal ("
              << normals_at_charges_locations[i] << ")" << std::endl;
    }

  deallog << "Exact potential normal gradient at point Q(0,0,0): "
          << exact_potential_normal_gradient << std::endl;
  ...
  // we use the AddNormDer(const double strength, const dealii::Point<3> &point,
  // const dealii::Point<3> &normal) member so that the expansion can account of
  // the effect of each charge considered
  for (unsigned int i = 0; i < charges_locations.size(); ++i)
    multipole.AddNormDer(charges_intensities[i],
                         charges_locations[i],
                         normals_at_charges_locations[i]);
  ...
  deallog << "The absolute multipole approximation error is: "
          << fabs(exact_potential_normal_gradient - approx_potential)
          << std::endl;

  deallog << "The relative multipole approximation error is: "
          << fabs(exact_potential_normal_gradient - approx_potential) /
               fabs(exact_potential_normal_gradient)
          << std::endl;
}
