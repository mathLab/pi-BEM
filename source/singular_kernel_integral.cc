...
SingularKernelIntegral<3>::SingularKernelIntegral(
  DoFHandler<2, 3>::active_cell_iterator in_cell,
...
SingularKernelIntegral<2>::SingularKernelIntegral(
  DoFHandler<1, 2>::active_cell_iterator in_cell,
          ...
          for (unsigned int q = 0; q < theta_q_points.size(); ++q)
            {
              ...
              Quadrature<2>  inner_uv_quadrature(inner_uv_q_points,
                                                inner_uv_q_weights);
              FEValues<2, 3> inner_face_fe_values(mapping,
                                                  fe,
                                                  inner_uv_quadrature,
                                                  update_values |
                                                    update_gradients |
                  ...
                  for (unsigned int ii = 0; ii < fe.dofs_per_cell; ++ii)
                    {
                      ...
                      //                          "<<fun*rho*inner_face_fe_values.JxW(p)<<
                      //                      "   arg2:
                      //                      "<<-F_2[0]/pow(rho,2)-F_1[0]/rho<<"
                      //                      diff:
                      //                      "<<fun*rho*inner_face_fe_values.JxW(p)-F_2[0]/pow(rho,2)-F_1[0]/rho<<std::endl;
...
SingularKernelIntegral<3>::evaluate_free_term_b()
{
  ...
  //  std::cout << "eta   " << eta[0] << ", " << eta[1] << ", " << eta[2]
  //            << std::endl;
  //  std::cout << "eta_n " << eta_n[0] << ", " << eta_n[1] << ", " << eta_n[2]
  //            << std::endl;
          ...
          for (unsigned int q = 0; q < theta_q_points.size(); ++q)
            {
              ...
              b += tmp * r_rho[q] * eta_g_norm / std::pow(r_rho_norm[q], 5.0) *
                   theta_fe_v.JxW(q);
