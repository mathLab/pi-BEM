...
main()
{
  ...
  // M. Guiggiani, A. Gigante,A general algorithm for multidimensional Cauchy
  // principal value integrals in the boundary element method, JOURNAL OF
  // APPLIED MECHANICS-TRANSACTIONS OF THE ASME,vol. 57,pp 906-915,tot.pag
  // 10,1990

  // strong test a
  Point<2> singularity_location_in_parametric_plane_1(0.6 * .5 + .5,
                                                      0.0 * .5 + .5);

  // strong test b
  Point<2> singularity_location_in_parametric_plane_2(0.6 * .5 + .5,
                                                      0.5 * .5 + .5);

  // strong test c
  Point<2> singularity_location_in_parametric_plane_3(-0.3 * .5 + .5,
                                                      0.2 * .5 + .5);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      deallog << "Result : " << I[0][0] * 4 * dealii::numbers::PI
              << "    Exact: " << -2.114175 << std::endl;
      deallog << "Abs Err: " << -2.114175 - I[0][0] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(-2.114175 - I[0][0] * 4 * dealii::numbers::PI) /
                   fabs(-2.114175)
              << std::endl;
      ...
      deallog << std::setprecision(15);
      deallog << "Result : " << I[0][0] * 4 * dealii::numbers::PI
              << "    Exact: " << -1.935711 << std::endl;
      deallog << "Abs Err: " << -1.935711 - I[0][0] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(-1.935711 - I[0][0] * 4 * dealii::numbers::PI) /
                   fabs(-1.935711)
              << std::endl;
      ...
      deallog << std::setprecision(15);
      deallog << "Result : " << I[0][0] * 4 * dealii::numbers::PI
              << "    Exact: " << .8790179 << std::endl;
      deallog << "Abs Err: " << .8790179 - I[0][0] * 4 * dealii::numbers::PI
              << std::endl;
      deallog << "Rel Err: "
              << fabs(.8790179 - I[0][0] * 4 * dealii::numbers::PI) /
                   fabs(.8790179)
              << std::endl;
    }
}
