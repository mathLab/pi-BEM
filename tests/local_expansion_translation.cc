...
main()
{
  ...
  // to use it compute a local expansion centered in Q'(2,2,2). The latter will
  // be used to approximate the potential in Q due to the charges in P_i
  ...
  // expansion previously created, using the Add(const MultipoleExpansion
  // &multipole) member function
  local.Add(multipole);


  // to test the local expansion translation, we create a second local expansion
  // centered in point Q''(-1,-1,-1)
  Point<3>       new_new_center(-1.0, -1.0, -1.0);
  LocalExpansion new_local(truncation_order, new_new_center, alf_ptr);


  // and fill the newest empty expansion with the contribution of the previously
  // created local expansion, making use of the Add(const LocalExpansion
  ...
  double approx_potential = new_local.Evaluate(zero);
