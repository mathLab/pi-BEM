                                                    ...
                                                    const VEC &src) const
{
  ...
  VEC                   localized_src;
  localized_src.reinit(parallel_partitioning, mpi_communicator);
  for (unsigned int i = 0; i < src.n_blocks(); ++i)
    {
      localized_src.block(i).reinit(src.block(i), false, true);
    }
      ...
      if (constraints.is_constrained(i))
        {
          ...
          for (const auto &entry : *constraints.get_constraint_entries(i))
            {
              dst.block(0)(i) -=
                entry.second * localized_src.block(0)(entry.first);
            }
          ...
          for (const auto &entry : *constraints_imag.get_constraint_entries(i))
            {
              dst.block(1)(i) -=
                entry.second * localized_src.block(1)(entry.first);
            }
        }
    }
